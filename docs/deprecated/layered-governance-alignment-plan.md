...
- 数据库表名单数化收口已完成：
  - 账号表当前为 `base_user_account`。
  - 异步任务记录表当前为 `base_async_task_record`。
  - 验证记录表当前为 `base_verification_record`。
  - AI provider 调用记录表当前为 `ai_provider_call_record`。
  - 已同步 entity、baseline migration、空库校验、约束错误识别和相关文档引用。
...
- D4：数据库物理表名统一使用单数；既有复数表名已在最后收口项处理完成，后续不得恢复。
